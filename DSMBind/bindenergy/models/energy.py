...
class AllAtomEnergyModel(FARigidModel):
    ...
    def __init__(self, args):
        ...
        self.density = [_density(exp, self.theta_range) for exp in self.expansion]
        self.score = [_score(exp, self.theta_range, sigma) for exp, sigma in zip(self.expansion, self.sigma_range)]
        self.register_buffer("eye", torch.eye(3).unsqueeze(0), persistent=False)
    ...
    def forward(self, binder, target, use_sidechain=True):
        ...
        sigma = torch.tensor([self.sigma_range[i] for i in sidx], device=device).float()
        tidx = [np.random.choice(list(range(100)), p=self.density[i]) for i in sidx]
        theta = torch.tensor([self.theta_range[i] for i in tidx], device=device).float()
        w = torch.randn(B, 3, device=device)
        hat_w = F.normalize(w, dim=-1)
        w = hat_w * theta.unsqueeze(-1)
        eps = np.random.uniform(0.1, 1.0, size=B)
        eps = torch.tensor(eps, device=device).float().unsqueeze(-1)
        hat_t = torch.randn(B, 3, device=device) * eps
        # Apply
        center = self.mean(true_X[:,:,1], bind_mask)
        bind_X = true_X - center[:,None,None,:]
        bind_X = self.rotate(bind_X, w) + hat_t[:,None,None,:]
        ...
        sigma = torch.tensor([self.sigma_range[i] for i in aidx], device=device).float()
        bidx = [np.random.choice(list(range(100)), p=self.density[i]) for i in aidx]
        theta = torch.tensor([self.theta_range[i] for i in bidx], device=device).float()
        u = torch.randn(B*N, 3, device=device)
        ...
        score = torch.tensor([self.score[i][j] for i,j in zip(sidx, tidx)], device=device).float()
        wloss = self.mse_loss(w, hat_w * score.unsqueeze(-1))
        tloss = self.mse_loss(t * eps, -hat_t / eps)
        # Side chain score matching loss
        score = torch.tensor([self.score[i][j] for i,j in zip(aidx, bidx)], device=device).float()
    ...
    def gaussian_forward(self, binder, target):
        ...
        eps = np.random.uniform(0.1, 1.0, size=B)
        eps = torch.tensor(eps, device=true_X.device).float()[:,None,None,None]
        hat_t = torch.randn_like(true_X) * eps
